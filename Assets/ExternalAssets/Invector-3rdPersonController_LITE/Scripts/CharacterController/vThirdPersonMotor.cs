...
namespace Invector.vCharacterController
{
    public class vThirdPersonMotor : MonoBehaviour
    {
        ...
        public void Init()
        {
            if (this.TryGetComponent<Character>(out Character character))
            {
                cc = character;
        ...
        private void CheckInput()
        {
            ...
            if (Input.GetKeyDown(InputDefine.Dance1Key) || Input.GetKeyDown(InputDefine.Dance2Key)
                || Input.GetKeyDown(InputDefine.Dance3Key) || Input.GetKeyDown(InputDefine.Dance4Key))
            {
                if (cc.isCloaked)
                    return;
                
                if(isDancing)
                    animator.SetTrigger(vAnimatorParameters.StopDancing);
                else
                {
                    if (Input.GetKeyDown(InputDefine.Dance1Key))
                    {
                        animator.SetTrigger(vAnimatorParameters.StartDance1);
                    }
                    else if(Input.GetKeyDown(InputDefine.Dance2Key))
                    {
                        animator.SetTrigger(vAnimatorParameters.StartDance2);
                    }
                    else if(Input.GetKeyDown(InputDefine.Dance3Key))
                    {
                        animator.SetTrigger(vAnimatorParameters.StartDance3);
                    }
                    else if(Input.GetKeyDown(InputDefine.Dance4Key))
                    {
                        animator.SetTrigger(vAnimatorParameters.StartDance4);
                    }
                }
                isDancing = !isDancing;
            }
            ...
            if (Input.GetMouseButtonDown(0))
            {
                ...
                if (camera.lockCamera)
                    return;
                if (cc.role == Define.Role.Observer)
                    return;
                Attack();
            }
            

            if (this.cc.role == Define.Role.Defender)
            {
                if (Input.GetKey(InputDefine.SkillKey))
                {
                    if (!isDefending)
                    {
                        isDefending = true;
                        animator.SetTrigger(vAnimatorParameters.StartDefense);
                        cc.isDefending.Value = true;
            ...
            if (Input.GetKeyDown(InputDefine.SkillKey))
            {
                RoleManager.Instance.skill.Invoke();
            }
        }

        public void FinishDance()
        {
            this.GetComponent<Character>().GetScore(Define.DANCE_SCORE);
            animator.SetTrigger(vAnimatorParameters.StopDancing);
            isDancing = false;
        }
        ...
        private void Attack()
        {
            animator.SetTrigger(vAnimatorParameters.IsAttack);
        }
        ...
        public void Death()
        {
            ...
            animator.SetTrigger(vAnimatorParameters.IsDeath);
            isDeath = true;

            if (this.TryGetComponent<Character>(out Character character))
            {
                ServerManager.Instance.KillCharacterServerRpc(character.ownerId);    
            }
        }
